...
# (csv column name -> strict phrase). Order here = column order in the CSV.
PHRASE_COLUMNS = [
...
def build(data_dir, out_path):
    cols = [(name, strict_pattern(phrase)) for name, phrase in PHRASE_COLUMNS]
    header = ID_COLUMNS + [name for name, _ in PHRASE_COLUMNS]
    tally = {name: 0 for name, _ in PHRASE_COLUMNS}
    ...
    return rows, skipped, errors, header, tally
...
def main():
    ...
    rows, skipped, errors, header, tally = build(data_dir, out_path)
